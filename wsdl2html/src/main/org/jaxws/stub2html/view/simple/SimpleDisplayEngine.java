 ...
 * 
 */
public class SimpleDisplayEngine extends DisplayEngine {

	private StringBuffer html;

	public SimpleDisplayEngine(WebMethodStubSet methodStubSet, JavaNameDisplayStrategy nameDisplayingStrategy) {
		super(methodStubSet, nameDisplayingStrategy);
	...
	private String displayTree(int indence, StubTypeTree tree) {
		...
		addNormalLineBreak(treeHtml, nameDisplayingStrategy.displayReadableClassName(tree.getType().getSimpleName()));
	...
	private String toRows(int indence, Stub stub) {
		...
		addNormalLineBreak(rows, toTD(nameDisplayingStrategy.displaySubType(stub)));
		addNormalLineBreak(rows, toTD(nameDisplayingStrategy.displayDataType(stub.getType())));
		addNormalLineBreak(rows, toTD(nameDisplayingStrategy.displayRequired(stub)));
		addNormalLineBreak(rows, toTD(nameDisplayingStrategy.displayMultiOccurs(stub)));
		addNormalLineBreak(rows, "</tr>");
